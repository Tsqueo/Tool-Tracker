...
const state = {
  ...
  toolJobFilter:null, specialToolFilter:null, syncingJobLocations:new Set()
};
...
function applyBranding(){const name=state.settings.businessName||'Tool Tracker';$('topbarSubtitle').textContent=name==='Tool Tracker'?'Live crew inventory':name;for(const id of ['topLogoWrap','dashboardLogo']){const el=$(id);el.innerHTML='<img src="tt-logo.png" alt="Tool Tracker logo" />'}}
...
function startRealtime(){
  ...
  sub(collection(db,'jobs'),snap=>{state.jobs=snap.docs.map(d=>({id:d.id,...d.data()}));fillJobSelects();renderJobs();updateStats();reconcileJobLocations().catch(console.error)},'Jobs');
...
function historyHTML(h){return`<div class="history-item"><strong>${esc(h.toolLabel||'')} ${esc(h.toolName||'')}</strong><div>${esc(h.action||'Updated')}${h.note?` — ${esc(h.note)}`:''}</div><time>${esc(formatPersonName(h.actorName||'Employee'))} · ${formatDate(h.createdAt)}</time></div>`}
...
function renderWishlist(){const rows=state.wishlist;$('wishlistList').innerHTML=rows.length?rows.map(w=>{const canEdit=isAdmin()||w.requestedBy===state.user.uid;return`<div class="info-card"><div class="info-card-head"><div><h3>${esc(w.name)}</h3><p>${esc(w.brand||'')}${w.reason?`${w.brand?' · ':''}${esc(w.reason)}`:''}</p></div><span class="badge ${slugStatus(w.status)}">${esc(w.status||'Requested')}</span></div><div class="info-meta"><span>${esc(w.priority||'Nice to have')}</span><span>Suggested by ${esc(formatPersonName(w.requestedByName||'Crew'))}</span><span>${formatDate(w.createdAt)}</span></div>${canEdit?`<div class="card-actions"><button class="btn smallbtn" data-edit-wish="${w.id}">Edit</button></div>`:''}</div>`}).join(''):'<div class="panel empty">No wishlist items yet. Crew suggestions will show here.</div>'}
...
function resetToolForm(){$('toolForm').reset();$('toolId').value='';$('toolStatus').value='Available';$('toolModalTitle').textContent='Add tool';$('privateTool').checked=false;const preferred=state.locations.find(l=>l.name==='Shop'&&l.active!==false)||activeLocations()[0];if(preferred)$('toolLocation').value=preferred.id;updateLabelSuggestion();$('toolLabel').value=$('labelSuggestion').textContent;if($('manufacturerPhotoFile'))$('manufacturerPhotoFile').value=''}
function editTool(id){const t=state.tools.find(x=>x.id===id);if(!t)return;$('toolId').value=t.id;$('toolBrand').value=t.brand||'';$('toolLabel').value=t.label||'';$('toolName').value=t.name||'';const matchedCategory=t.categoryId||state.categories.find(c=>String(c.name||'').toLowerCase()===String(t.categoryName||t.category||'').toLowerCase())?.id||'';$('toolCategory').value=matchedCategory;$('toolModel').value=t.model||'';$('toolSerial').value=t.serial||'';$('toolStatus').value=t.status||'Available';$('toolLocation').value=t.locationId||'';$('purchaseDate').value=t.purchaseDate||'';$('warrantyExpiry').value=t.warrantyExpiry||'';$('manufacturerPhotoURL').value=t.manufacturerPhotoURL||'';$('privateTool').checked=!!t.privateTool;$('toolNotes').value=t.notes||'';$('toolPhoto').value='';if($('manufacturerPhotoFile'))$('manufacturerPhotoFile').value='';$('toolModalTitle').textContent='Edit tool';updateLabelSuggestion();openModal('toolModal')}
function viewTool(id){
  const t=state.tools.find(x=>x.id===id);if(!t||(!isAdmin()&&t.privateTool))return;
  const cat=t.categoryName||t.category||'Uncategorized',w=warrantyState(t.warrantyExpiry);const photos=[];
  if(t.manufacturerPhotoURL)photos.push(`<button class="detail-photo-wrap photo-button" type="button" data-preview-image="${esc(t.manufacturerPhotoURL)}" data-preview-title="Manufacturer / Primary"><img class="detail-photo" src="${esc(t.manufacturerPhotoURL)}" alt="Manufacturer photo"><span class="photo-slot-label">Manufacturer / Primary</span><small>Tap to enlarge</small></button>`);
  if(t.photoURL)photos.push(`<button class="detail-photo-wrap photo-button" type="button" data-preview-image="${esc(t.photoURL)}" data-preview-title="Our Tool"><img class="detail-photo" src="${esc(t.photoURL)}" alt="Actual tool photo"><span class="photo-slot-label">Our Tool</span><small>Tap to enlarge</small></button>`);
  const hist=state.history.filter(h=>h.toolId===id).slice(0,10);
  $('detailTitle').textContent=`${t.label||''} · ${t.name||'Tool'}`;
  $('toolDetailBody').innerHTML=`<div class="detail-hero"><div class="detail-photo-grid">${photos.length?photos.join(''):`<div class="tool-placeholder" style="width:100%;min-height:180px">${icon('tools')}</div>`}</div><div><div class="card-actions"><span class="badge ${slugStatus(t.status)}">${esc(t.status||'Available')}</span>${t.privateTool?'<span class="badge">Private</span>':''}</div><h2>${esc(t.name||'Unnamed tool')}</h2><p class="muted">${esc(t.brand||'')} ${esc(t.model||'')}</p><div class="detail-data"><div><span>Label</span><strong>${esc(t.label||'—')}</strong></div><div><span>Location</span><strong>${esc(t.locationName||'Unassigned')}</strong></div><div><span>Category</span><strong>${esc(cat)}</strong></div><div><span>Serial</span><strong>${esc(t.serial||'—')}</strong></div><div><span>Purchased</span><strong>${formatDateOnly(t.purchaseDate)}</strong></div><div><span>Warranty</span><strong>${t.warrantyExpiry?`${formatDateOnly(t.warrantyExpiry)}${w?` · ${w.label}`:''}`:'—'}</strong></div></div><p>${esc(t.notes||'')}</p><div class="card-actions"><button class="btn accent" data-move-tool="${t.id}">Move / status</button><button class="btn" data-edit-tool="${t.id}">Edit Tool</button></div></div></div><hr style="border:0;border-top:1px solid var(--line);margin:18px 0"><h3>Recent history</h3><div class="timeline">${hist.length?hist.map(historyHTML).join(''):'<div class="empty">No history yet.</div>'}</div>`;
...
document.addEventListener('click',e=>{
  const button=e.target.closest('button,[data-open-view],[data-view-tool],[data-move-tool],[data-edit-tool],[data-edit-job],[data-job-tools],[data-edit-location],[data-edit-category],[data-edit-wish],[data-category-up],[data-category-down],[data-remove-prefix],[data-preview-image]');if(!button)return;
